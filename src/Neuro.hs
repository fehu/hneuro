...
newNeuron id w wf f     = Neuron id w wf f
...
instance Show a => Show (NetworkElem a) where
    show (Neuron id w wf f)         = "neuron:" ++ show id ++ "(" ++ show wf ++ " " ++ show w ++ ", "
                                                                  ++ show f ++ ")"
...
--    (Neuron id1 _ _ _)      `compare` (Neuron id2 _ _)       = id1 `compare` id2
...
mkSynapsesCache syns = SynapsesCache{ fromMap = fromListWith (++)           pars
                     ...
                     where pars = map asPairSeq syns

getTo   SynapsesCache {toMap   = m} id = m ! id
getFrom SynapsesCache {fromMap = m} id = m ! id

asPair    (Synapse from to) = (from, to)
asPairSeq (Synapse from to) = (from, [to])
